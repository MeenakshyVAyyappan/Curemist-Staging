...
function InfoCard({ icon, title, items, accent = false }: InfoCardProps) {
  return (
    ...
    >
      ...
      >
        <span className={accent ? "text-brand-yellow" : "text-brand-blue"}>
          {icon}
        </span>
      </div>
      <h3
        className={`font-bold text-base md:text-lg mb-4 ${accent ? "text-white" : "text-brand-blue"}`}
      >
        {title}
      </h3>
...
const shippingCards = [
  ...
  },
  {
    ...
    items: [
      "Free shipping on all orders throughout India.",
      "International shipping is charged on a case-to-case basis. Please contact customer support for more details.",
    ],
  },
  {
      ...
      <section className="bg-gray-50 py-12 md:py-20">
        <div className="container mx-auto px-4 md:px-6 lg:px-24">
          ...
          <div className="bg-gradient-to-r from-brand-yellow/30 to-brand-yellow/10 border border-brand-yellow/40 rounded-2xl p-5 md:p-7 mb-10 flex flex-col sm:flex-row items-start sm:items-center gap-3">
            ...
            <div>
              <p className="text-brand-blue font-bold text-sm md:text-base">
                Free Shipping Across India
              </p>
              <p className="text-gray-600 text-xs md:text-sm mt-0.5">
                We ship PAN India. Delivery within 4–7 business days. Returns
                accepted within 7 days for damaged products.
              </p>
            </div>
          </div>
