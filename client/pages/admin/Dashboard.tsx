...
export default function AdminDashboard() {
  ...
  const defaultStartDate = startDateDefault.toISOString().slice(0, 10);
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // 1. Fetch Orders for Stats and Chart (range filter)
      ...
      // 2. Fetch Users Count with same date filter (signups in window)
      ...
      const chartDataFormatted = Object.keys(revenueByDate)
        .map((date) => ({
          date,
          revenue: revenueByDate[date],
        }))
        .slice(0, 7)
        .reverse(); // Last 7 unique days (approx)
