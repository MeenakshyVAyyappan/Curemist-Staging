...
export default function Checkout() {
  ...
  const finalizeOrderStatus = async (status: 'paid' | 'payment_failed', razorpayPaymentId?: string) => {
    ...
    const { error } = await supabase.from('orders').update(updateData).eq('id', currentOrderId);
    if (error) {
  ...
  const handlePlaceOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLoading(false);
      setProcessingPayment(false);
      toast({ title: 'Payment Error', description: err.message || 'Could not create order before payment', variant: 'destructive' });
    }
  };
      ...
      {(processingPayment || loading) && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="max-w-md rounded-lg bg-white p-6 text-center shadow-lg">
            <h2 className="text-xl font-bold mb-2 text-gray-900">Order processing, please wait...</h2>
            <p className="text-sm text-gray-700 mb-3">Do not click back or refresh. Your payment is being completed.</p>
            <p className="text-sm text-gray-600">You will be redirected to your order history automatically.</p>
          </div>
        </div>
      )}
          ...
          <div className="grid lg:grid-cols-3 gap-6 md:gap-8">
            ...
            <div className="lg:col-span-2">
              <form onSubmit={handlePlaceOrder} className="space-y-8">
                ...
                >
                  {loading || processingPayment ? 'Processing...' : `Pay ₹${totalPrice} Now`}
                </button>
