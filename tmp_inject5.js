...
content = content.replace(
  /(value=\{billingAddress\.street\}[^>]*placeholder="Street Address"\s*)\/>/g,
  "$1required\n                        />",
);
content = content.replace(
  /(value=\{billingAddress\.city\}[^>]*placeholder="City\/Town"\s*)\/>/g,
  "$1required\n                          />",
);
content = content.replace(
  /(value=\{billingAddress\.state\}[^>]*placeholder="State\/Province"\s*)\/>/g,
  "$1required\n                          />",
);
content = content.replace(
  /(value=\{billingAddress\.zip\}[^>]*placeholder="ZIP\/Postal Code"\s*)\/>/g,
  "$1required\n                          />",
);
content = content.replace(
  /(value=\{billingAddress\.country\}[^>]*placeholder="Country"\s*)\/>/g,
  "$1required\n                          />",
);
...
function addErrorTextToPhone(currentContent) {
  ...
  return currentContent.replace(
    /(<input[\s\S]*?placeholder="Phone Number"[\s\S]*?\/>\s*)\{customerInfo\.phone\.length > 0[\s\S]*?<\/div>/g,
    `$1${replStr}`,
  );
}
...
// Select Gender
content = content.replace(
  /(<select[\s\S]*?value=\{customerInfo\.sex\}[\s\S]*?<\/select>)/g,
